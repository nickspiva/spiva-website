...
- [ ] **Sidebar preset button active states** — when a preset scenario is selected (Current Planned / Mayor's Original NRI / Mayor's Updated NRI), the corresponding button should fill with its border color and display white text, making it visually clear which scenario is active. When the user has customized individual TAD sliders to a configuration that doesn't match any preset, all three buttons revert to their unfilled outline state. Implementation: track a `active_preset` reactive value in the server (set on button click, cleared when any slider moves away from preset values); use `shinyjs` or custom JS to toggle Bootstrap button classes (`btn-primary` / `btn-outline-primary`, etc.) based on that reactive.
- [ ] **Revenue diverted chart — show $0B baseline** — the y-axis currently auto-fits to the data range, hiding the $0 line. Add `limits = c(0, NA)` to `scale_y_continuous()` so the axis always starts at zero. This makes the cumulative growth from a true zero baseline visually clear.
- [ ] **Revenue diverted / projected APS revenue toggle** — explore making the revenue diverted card toggelable: a tab or toggle button in the card header lets users switch between (a) the cumulative diversion chart (3 fixed scenario lines, what schools are losing while TADs are open) and (b) the projected annual APS revenue chart (per-TAD lines, what schools gain as TADs close). These two views are two sides of the same policy question and sharing a card would reinforce that relationship while saving vertical space. Implementation: `bslib::navset_card_tab` or a toggle button + `conditionalPanel`. Note that the two charts have different interactivity — the diversion chart shows fixed scenarios while the projection chart responds to sliders — so the card header context may need to change accordingly when toggled.
